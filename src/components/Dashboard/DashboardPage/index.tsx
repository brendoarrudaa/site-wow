...
const DashboardPage = ({ username, email, gmLevel }: DashboardPageProps) => {
          ...
          {loading ? (
            ...
            <div className="card border border-dashed border-base-300 bg-base-100 p-8 text-center text-base-content/40">
              ...
              <p className="mt-1 text-xs">
                Entre no jogo e crie seu primeiro personagem!
              </p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {characters.map(character => {
                ...
                const faction = HORDE_RACES.has(character.race)
                  ? 'Horda'
                  : 'Aliança'
                ...
                return (
                  <div
                    key={character.guid}
                    className="card-fantasy-hover relative overflow-hidden"
                  >
                    {/* Class color accent bar */}
                    <div
                      className={`absolute left-0 top-0 h-full w-1 ${accentBg}`}
                    />
