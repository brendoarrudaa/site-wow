...
const SERVICES: ServiceDef[] = [
  ...
  },
  {
    ...
    name: 'Troca de Raça',
    description:
      'Mude para outra raça da mesma facção. Aparência personalizada no próximo login.',
    icon: Users,
    mode: 'direct',
    directLabel: 'Aplicado na hora'
  },
  {
    ...
    description:
      'Mude de Horda para Aliança ou vice-versa. Guild removida automaticamente.',
    icon: Repeat,
    mode: 'direct',
    directLabel: 'Aplicado na hora'
  }
...
const ServicosPage = () => {
  ...
  const faction = character
    ? HORDE_RACES_NAMES.has(character.race)
      ? 'Horda'
      : 'Aliança'
    : null
  ...
  const validForClass = character
    ? (CLASS_RACE_MAP[character.classId] ?? new Set<number>())
    : new Set<number>()

  // Available races for race-change (same faction, excluding current, compatible with class)
  const sameFactRaces = character
    ? (HORDE_RACE_ID_SET.has(character.raceId)
        ? HORDE_RACE_IDS
        : ALLIANCE_RACE_IDS
  ...
  const openService = (svcId: string) => {
    ...
    if (svcId === 'race-change' && character) {
      ...
      setNewRace(
        FACTION_CHANGE_MAP[character.raceId] ?? oppFactRaces[0] ?? null
      )
    } else {
      setNewRace(null)
    }
  ...
  const handleExecute = async () => {
    ...
    if (confirmService === 'race-change' || confirmService === 'faction-change')
      body.newRace = newRace
    ...
    try {
      ...
      if (!res.ok) {
        setResult({
          success: false,
          message: data.error || 'Erro ao executar serviço.'
        })
      } else {
        setResult({ success: true, message: data.message })
        if (
          ['name-change', 'race-change', 'faction-change'].includes(
            confirmService
          )
        ) {
          loadCharacters()
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            ...
            {character && (
              <div className="flex items-center gap-3 rounded-lg border border-base-300 bg-base-200/50 px-4 py-2">
                ...
                {character.online && (
                  <span className="badge badge-success badge-sm p-2 ml-1 text-[10px]">
                    Online
                  </span>
                )}
              </div>
            ...
            >
              ...
              {(() => {
                const noRaces =
                  (svc.id === 'race-change' &&
                    character &&
                    sameFactRaces.length === 0) ||
                  (svc.id === 'faction-change' &&
                    character &&
                    oppFactRaces.length === 0)
                return (
                  <button
                    ...
                    title={
                      noRaces
                        ? 'Nenhuma raça compatível com esta classe'
                        : undefined
                    }
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-base-content/60">
                A aparência é resetada e pode ser personalizada no próximo login
                no jogo.
              </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                {confirmService === 'level-reset' && (
                  <div className="flex items-start gap-3 rounded-lg border border-warning/50 bg-warning/10 p-3 text-sm">
                    <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-warning" />
                    <span>
                      Todos os talentos serão removidos. Você poderá
                      redistribuí-los no jogo.
                    </span>
                  </div>
                )}
                {confirmService === 'faction-change' && (
                  <div className="flex items-start gap-3 rounded-lg border border-error/50 bg-error/10 p-3 text-sm">
                    ...
                    <span>
                      Você será removido da sua guild. Reputações com facções
                      opostas serão mantidas, mas NPCs inimigas podem atacar.
                    </span>
                  </div>
                )}
                {(confirmService === 'race-change' ||
                  ...
                  <div className="flex items-start gap-3 rounded-lg border border-info/50 bg-info/10 p-3 text-sm">
                    ...
                    <span>
                      A aparência será resetada. Você poderá personalizar no
                      próximo login no jogo.
                    </span>
                  </div>
                )}
                {character.online && (
