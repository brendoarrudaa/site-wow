  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`}>
      ...
      >
        ...
        <span className="hidden sm:inline font-medium">{locale.toUpperCase().replace("-", "")}</span>
        <ChevronDown className={`h-3 w-3 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="absolute right-0 top-full mt-2 w-40 rounded-lg border border-border/80 bg-card/95 backdrop-blur-xl p-1.5 shadow-xl z-50">
