    ...
    >
      <div className="page-container flex h-16 items-center justify-between">
          ...
          {navCategories.map(cat => (
            <div key={cat.labelKey} className="relative">
              <button
                onClick={() =>
                  setOpenCategory(
                    openCategory === cat.labelKey ? null : cat.labelKey
                  )
                }
              ...
              >
                {t(cat.labelKey)}
                <ChevronDown
                  className={`h-3 w-3 transition-transform ${openCategory === cat.labelKey ? 'rotate-180' : ''}`}
                />
              </button>
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          <Link
            href="/cadastro"
            className="btn btn-sm btn-outline border-gold/60 text-gold hover:bg-gold hover:text-black hover:border-gold"
          >
            {t('nav.register')}
          </Link>
          <Link
            href="/como-jogar"
            className="btn btn-sm bg-gold text-black hover:bg-gold/90 border-0"
          >
            {t('nav.playNow')}
          </Link>
        </div>
          ...
          <nav className="page-container py-4 flex flex-col gap-1">
            ...
            {navCategories.map(cat => (
              <div key={cat.labelKey}>
                ...
                >
                  <span className="font-medium text-foreground/70 uppercase text-xs tracking-wider">
                    {t(cat.labelKey)}
                  </span>
                  <ChevronDown
                    className={`h-3 w-3 transition-transform ${openCategory === cat.labelKey ? 'rotate-180' : ''}`}
