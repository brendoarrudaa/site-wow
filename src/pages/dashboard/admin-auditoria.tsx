...
const QUICK_FILTERS: { label: string; actions: string[] }[] = [
  {
    label: 'Leilões',
    actions: ['CREATE_AUCTION', 'AUCTION_PUBLISHED', 'AUCTION_EDITED',
              'AUCTION_CANCELLED', 'AUCTION_CLOSED_GM', 'AUCTION_WIN',
              'BID_PLACED', 'BID_REFUND'],
  },
  {
    label: 'Marketplace',
    actions: ['CREATE_LISTING', 'APPROVE_LISTING', 'REJECT_LISTING',
...
function ExpandableJson({ label, value }: { label: string; value: string | null | undefined }) {
...
export default function AuditoriaPage({ user }: { user: SessionUser }) {
  ...
  const clearFilters = () => {
    setFilters({ action_type: '', account_id: '', date_from: '', date_to: '', page: 1 })
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-primary">Log de Auditoria</h1>
              <p className="text-base-content/60 text-sm mt-0.5">
                {total > 0 ? `${total.toLocaleString('pt-BR')} registros encontrados` : 'Histórico de ações do marketplace'}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-xs text-base-content/50 font-semibold uppercase tracking-wider mr-1">Filtros rápidos:</span>
          ...
          <div className="card-body py-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="form-control">
                <label className="label py-1"><span className="label-text text-xs">Tipo de Ação</span></label>
                <select
                ...
                >
                  ...
                  <optgroup label="Leilão">
                    ...
                    <option value="AUCTION_CLOSED_GM">Fechar Leilão (GM)</option>
              ...
              <div className="form-control">
                <label className="label py-1"><span className="label-text text-xs">Account ID</span></label>
                <input
                  type="number"
                  placeholder="Ex: 1234"
              ...
              <div className="form-control">
                <label className="label py-1"><span className="label-text text-xs">Data Inicial</span></label>
                <input
                  type="date"
                  className="input input-bordered input-sm"
              ...
              <div className="form-control">
                <label className="label py-1"><span className="label-text text-xs">Data Final</span></label>
                <input
                  type="date"
                  className="input input-bordered input-sm"
            ...
            <div className="flex gap-2 mt-3">
              <button className="btn btn-primary btn-sm gap-2" onClick={applyFilters}>
          ...
          <div className="card-body p-0">
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-base-content/50">Nenhum registro encontrado</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="table table-xs w-full">
                  ...
                  <tbody>
                    {logs.map((log, idx) => {
                      const isCritical = log.is_critical || CRITICAL_ACTIONS.includes(log.action_type)
                      ...
                      return (
                        <tr
                          key={log.id}
                          className={[
                            isCritical ? 'bg-warning/5 border-l-2 border-l-warning' : '',
                        ...
                        >
                          ...
                          {/* Papel */}
                          <td className="align-top pt-3">
                            <span className={`badge badge-xs ${roleColor(log.actor_role ?? 'PLAYER')}`}>
                          ...
                          {/* Ação */}
                          <td className="align-top pt-3">
                            <span className={`badge badge-sm font-mono text-xs ${actionColor(log.action_type)}`}>
                              {log.action_type}
                            </span>
                          </td>
                          ...
                          <td className="align-top pt-2">
                            <div className="space-y-1">
                              <ExpandableJson label="before" value={log.before_state} />
                              <ExpandableJson label="after" value={log.after_state} />
                              <ExpandableJson label="details" value={log.details} />
                            </div>
                          </td>
                          ...
                          <td className="align-top pt-2">
                            <button
                              className="text-xs font-mono text-base-content/50 hover:text-base-content flex items-center gap-1"
                              ...
                              <div className="mt-1 text-xs bg-base-300 rounded p-2 max-w-[200px] space-y-1">
                                {log.user_agent && (
                                  <p className="text-base-content/60 break-all">
                                    <span className="font-semibold text-base-content/80">UA: </span>
                                    {log.user_agent}
                                  </p>
                                )}
                                {log.correlation_id && (
                                  <p className="font-mono text-base-content/60 break-all">
                                    <span className="font-semibold text-base-content/80">Corr: </span>
                                    {log.correlation_id}
                                  </p>
                                )}
                              </div>
        ...
        <div className="alert alert-warning alert-soft text-sm">
          ...
          <div>
            <strong>Ações críticas</strong> são destacadas com borda laranja e ícone{' '}
            <AlertTriangle className="h-3 w-3 inline text-warning" />.
            Incluem: aprovações, rejeições, entregas, estornos, créditos e fechamento de leilão.
          </div>
        </div>
      </div>
    </DashboardLayout>
